...
```{r q9_1_a}
models <- with(adults,
     svyglm(overwt ~ age, family = quasibinomial))

summary(MIcombine(models), digits = 3)
```

In the above table we see a statistically significant positive relationship
between age and obesity, even after accounting for the missing data.
  
  * Using just the complete data before imputation.
  
```{r q9_1_b}
load('data/nhanes/nhanes3.rda')
complete_design <- as_tibble(nhanes3) %>% 
  mutate(
    bmi = (BMPWTLBS * 0.454)/ (BMPHTIN * 2.54 / 100)^2,
    overwt = if_else(HSSEX == 1, bmi >= 27.8, bmi >= 27.3),
    age = HSAGEIR) %>% 
  ## restrict to adults
  filter(age > 20) %>% 
  mutate(missing =  is.na(overwt) | is.na(age)) %>% 
  as_survey_design(id = SDPPSU6, strata = SDPSTRA6,
                   weights = WTPFHX6, 
                   subset = I(!missing),
                   nest = TRUE,
                    data = nhanes3)
model <- svyglm(overwt ~ age, family = quasibinomial, design = complete_design)
summary(model)
```
...
inference in the context of survey data.
