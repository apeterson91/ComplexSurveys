...
categorical data. Though the `survey` package also handles poisson and
binomial regression neither of these are covered in this chapter.
...
association the only change to `svyglm()` is adding the `family=quasibinomial()`
option.
...
plot(svysmooth(intuse ~ age, design = subset(shs, sex == "male" & !is.na(age)),
...
Lumley doesn't go too much farther beyond identifying positive or negative 
associations for the different parameters, so I'll refrain from offering too
...
2. Post-stratification is a rgression estimator, where each post-stratum is 
...
From this starting point, Lumley descibres that calibration follows a similar 
