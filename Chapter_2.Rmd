...
Interview Survey [data](http://healthpolicy.ucla.edu/chis/data/Pages/GetCHISData.aspx) he uses in the text but I've pasted the code below as it seems straightforward enough.
...
the `fivesurv` function...

### Now again with the `srvyr` package
```{r}
boot_design <- as_survey_design(strat_design, 
                                type = "bootstrap", 
                                replicates = 100)
boot_design
```
