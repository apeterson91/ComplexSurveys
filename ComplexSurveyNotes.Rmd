...
wa_crime_df <- readxl::read_xlsx("data/WA_crime/1984-2011.xlsx", skip = 4) %>%
  ...
  select(
    ...
    property_crime, num_counties, num_agencies
  )
...
poisson_fit <- svyglm(murder ~ I(log(Population)), family = quasipoisson, 
                      design = srs_design)
summary(poisson_fit)
```
...
predict(svyratio(~murder, ~Population,
                 design = srs_design),
        total = sum(wa_crime_df$Population))
```
...
nhanes <- nhanes %>% 
  mutate(
    ...
    ), levels = c("Normal", "PreHypertension", "Hypertension Stage 1",
...
1. Using the WA State Criem population, take a stratified random sample of five
...
using the calibrated and uncalibrated sample.

* b. Convert the original survey design object to use jacknife replicate weights.
Calibrate the replicate-weight design using the same auxiliary variables and 
estimate the number of burglaries and of murders in the state.

* c. Calibrate the sample using the population and number of burgarlies in the
previous year as auxiliary variables, and estimat the number of burglaries and 
murders in the state.

* d. Estimate the ratio of violent crimes to property crimes in the state, 
using the uncalibrated sample and the sample calibrated on population and 
...
3. Write an R function taht accepts a set of 50 elephant weights and performs
the ranked set sampling procedure on page 150 to choose three of them. By
simulation, compare the bias and variance of the stimated total from ranked-set
