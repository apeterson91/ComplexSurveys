...
I'm imaging two general cases:

1. I intend to use them as a quick reference on any of the code or topics that
I may want to look up again in the future quickly. Or as a starting point for
related work.
...
This dataset comes from the The Mothers' and Children's Morbidity Study which
...
Lumley produces stabilized weights by averaging two model's respective 
probabilities of stress. A model (1) that includes the child's current and previous 
state of illness (which might cause subsequent stress on the mother's part), 
as well as the current day in the study and baselines stress and a second (2)
that looks only at the baseline stress and change in mother's stress over time.
