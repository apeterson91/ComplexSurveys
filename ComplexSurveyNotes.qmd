...
comments:
  giscus: 
    repo: apeterson91/ComplexSurveys
editor: visual
---
...
Guide to Analysis in R" [@lumley2011complex] and some of the methods underlying
...
[book](https://r-survey.r-forge.r-project.org/survey/). Some data sets are not
available, or at least I was not able to find them, or the the documentation for
the data was not available. I remark on this when relevant in my notes below.
...
double check Lumley's code or your understanding of the principles tested in
the exercises. 
...
```{r multistage_diagram, echo = FALSE}
grViz("
digraph dot {
graph [layout = dot]
node [shape = circle,
...
```{r q3_2_b_pps_stderrs, eval = FALSE}
...
```{r q_3_9_simulated_pps, cache=TRUE}
OneSimulation <- function() {
  insample <- sampling::UPtille(election$p)
  ppsample <- election[insample == 1, ]
  ppsample$wt <- 1 / ppsample$p
...
```{r q_3_9_fpc_pps_totals, cache=TRUE}
estimated_totals <- as_tibble(t(rowMeans(total_sims[, 1, ]))) %>%
  mutate(label = "Estimate")
...
```{r q_3_9_pps_stderrs, eval = TRUE, cache=TRUE}
estimated_stderrs <- as_tibble(t(rowMeans(total_sims[, 2, ]))) %>%
  mutate(label = "Estimate")
...
```{r 39_pps_cis, eval = TRUE, cache=TRUE}
totals <- unlist(true_totals[, 1:3])
prop_contained <- rowMeans((total_sims[, 3, ] < totals &
  total_sims[, 4, ] > totals) * 1)
prop_contained
...
population value of `r round(mean(apipop$api00), 2)`. Though it is worth
