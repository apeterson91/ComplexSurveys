...
wa_crime_df %>% 
  ...
  as_survey_design() %>% 
  summarize(total = survey_total(murder_and_crime, vartype = "ci"))
```
...
```{r wacrime_36b}
county_list[-which(county_list == "King" | county_list == "KING")]
```
...
3) Take a simple random sample of counties and include all the police districts.
...
(`thtrninc`)? For those households that did not receive benefits what mean
...
distribution and not the shape itself).
...
across the population(s) represented by our dataset. The naive, biased 
population that our sample represents when unweighted, or the "target" 
population our sample represents when reweighted. A follow-up question would
...
* Take a simple random sample of ten countiesfrom the state and use all the 
data from the sampled counties. Estimate the total number of murders and 
burglaries in the state.
...
* Stratify the sampling so that King County is sampled with 100% probability 
 ...
 as an auxiliary variable to construct a commmon ratio estimate and a separate
...
variable to construct a commmon ratio estimate and a separate ratio estimate of 
...
* What confouding variables measuring socioeconomic status of students should be 
included in the model?

* Should 1999 API be in hthe model (and why, or why not?)
...
because the data for the joint distribution - or cross classification as Lumley
calls it - is not available, or because one of the cross - strata have no
...
two, conditional on being sampled in phase one.
