...
pop_03 <- wa_crime_03_df %>% summarize(P = sum(Population)) %>% pull(P)

previous_year_calibration <- calibrate(sample_design,
                                       formula = ~ burglaries_03,
                                       population = c("(Intercept)" = pop_size,
...
```{r q7_1d_, eval = FALSE}
svyratio(numerator = ~violent_crime_04, 
...
results %>% 
  ...
  mutate(bias = average_estimate - true_total,
         truth = true_total)
```
...
results <- as_tibble(t(replicate(1000,
...
brfss_ca <- tbl(db,sql("SELECT * FROM brfss")) %>% filter(X_STATE == 6) %>% 
  collect() %>% 
  mutate(
    BMI_Cat = case_when(
...
postStratify(design = brfss_ca, strata = ~SEX + X_AGE_G, 
...
d_esoph <- svydesign(id = ~1, strata = ~case, weights = ~weight, 
                     data = esoph.x)
...
```{r esoph_demo_2}
tbl_merge(tbls = list(tbl_regression(unwtd), tbl_regression(wtd)),
...
ccs_design <- twophase(id = list(~1, ~1), subset = ~in.ccs,
                       strata = list(NULL, ~ relaps), data = nwts)
m1 <- svyglm(relaps ~ histol * stage + age + tumdiam, design = nwts_design,
             family = quasibinomial())
m2 <- svyglm(relaps ~ histol * stage + age + tumdiam, design = ccs_design, 
             family = quasibinomial())
m3 <- glm(relaps ~ histol * stage + age + tumdiam, data = nwts, subset = in.ccs,
          family = binomial())
m1a <- svyglm(relaps ~ histol * stage + age + tumdiam, design = nwts_design,
              family = quasipoisson(log))
m2a <- svyglm(relaps ~ histol * stage + age + tumdiam, design = ccs_design,
              family = quasipoisson(log))
```

```{r}
tbl_merge(tbls = list(tbl_regression(m1, conf.int = FALSE),
          ...
          tab_spanner = c("Two Phase Design based Odds Ratio",
...
cch_design <- twophase(id = list(~1, ~1), subset = ~in.cchsample,
...
s2 <- cch(Surv(trel, relaps) ~ histol * stage + age + tumdiam, id = ~id,
    data = cch_data, subcoh = ~in.subcohort, cohort.size = 3915)
tbl_merge(tbls = list(tbl_regression(s1), tbl_regression(s2)), 
          tab_spanner = c("**Case-Cohort Analysis**", 
...
scch_design <- twophase(id = list(~1, ~1), subset = ~in.cchsample,
                        strata = list(NULL, ~relaps), data = nwts)
...
s3 <- coxph(Surv(trel, relaps) ~ histol * stage + age + tumdiam,
            data = nwts)
tbl_merge(
  tbls = list(tbl_regression(s1),
              tbl_regression(s2),
              tbl_regression(s3)),
  c("**Stratified Case Cohort**",
...
var_cal <- calibrate(clus1_design, formula = ~api99 + ell + mobility + emer,
...
clus1if <- update(clus1_design, ifint = inffun[index, 1],
                  ifell = inffun[index, 2], ifmobility = inffun [ index, 3], 
                  ifemer = inffun[index, 4])
if_cal <- calibrate(clus1if, formula = ~ifint + ifell + ifmobility + ifemer,
                    pop = c(6194, 0, 0, 0 ,0))

m2 <- svyglm(api00 ~ ell + mobility + emer, design = if_cal)
tbl_merge(
  tbls = list(tbl_regression(m0, intercept = TRUE),
...
tbl_merge(
  tbls = list(tbl_regression(m1),
              tbl_regression(m2),
              tbl_regression(m3),
              tbl_regression(m4)
              ),
  tab_spanner = c("2-Phase Weighted",
                  "2-Phase Raked",
                  "2 Phase Imputed",
                  "Full Data")
)
...
the superpopulation probabilities for the cells in the 2 x 2 table match those
in Table 8.3.
...
data for 2003 as the auxiliary variables.
  ...
  *  Take a simple random sample of 20 police districts from the state and use 
  all the data from the sampled counties. Estimate teh ratio of violent crimes
  to property crimes.
